      ...
      const response = await fetch(
        "https://api.groq.com/openai/v1/chat/completions",
        {
          ...
          body: JSON.stringify({
            ...
            messages: [{ role: "user", content: prompt }],
            temperature: 0.2,
            max_tokens: 7000, // ✅ increased from 4096 — prevents JSON truncation
          }),
        }
      );
      if (!response.ok) {
...
const sanitizeJSON = (text) => {
  ...
  // Only replace control characters inside quoted strings
...
// ✅ Uses sanitizeJSON before parsing — fixes quiz + CA parse errors
...
const fetchAndStoreRawNews = async () => {
  ...
  for (const url of sources) {
    if (allItems.length >= 100) break; // ✅ stop early
    try {
      const feed = await rssParser.parseURL(url);
      let count = 0;
      for (const item of feed.items || []) {
        if (count >= 15) break; // ✅ max 15 per source
        ...
        if (!pubDate || isToday || isRecent) {
          seen.add(item.title);
          // ✅ Clean control characters at fetch time — prevents JSON parse errors later
  ...
  const capped = allItems.slice(0, 100); // ✅ final cap
  await db
    .collection("raw_news")
    .updateOne(
      { date: today },
...
// ✅ Simplified — removed quote-escaping instructions that caused model to output \" breaking JSON
...
// ── Hinglish generation — Gemini batches, Groq fallback ──────────────────────
const generateHinglishInBatches = async (rawItems, exam, today) => {
  ...
  // Split items into 3 equal parts — one per Gemini key
  ...
  // Call all 3 keys simultaneously
  const results = await Promise.all(
    chunks.map(async (chunk, i) => {
      ...
      // Direct call with specific key — no rotation
      try {
        const response = await fetch(
          `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-lite:generateContent?key=${key}`,
          {
        ...
        // Fallback to Groq for this chunk
...
// ── On-demand generation — DB lock ensures only 1 AI call ever ───────────────
...
// ── Admin: force refresh for testing ─────────────────────────────────────────
...
app.post("/quiz/generate", async (req, res) => {
  ...
  try {
    ...
    // ✅ extractJSONArray now sanitizes control characters before parsing
